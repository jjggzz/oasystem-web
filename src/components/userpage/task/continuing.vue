          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button
                ...
                type="warning"
                plain
                @click="cancelApply(scope.$index, scope.row)">取消</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-dialog
...
export default {
  computed: {
    showList:function(){
      return this.applyList.filter((item)=>{
        if((item.applyState==1) &&(item.applyTitle.includes(this.search)||item.flow.flowName.includes(this.search))){
          return item
        }
  ...
  methods: {
    ...
    cancelApply(index, row){
      this.$confirm('此操作将取消该任务！', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          //取消操作
          this.axios.put("/apply/"+row.applyId,{applyState:0})
          .then((res)=>{
